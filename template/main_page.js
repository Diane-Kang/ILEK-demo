...
async function main() {
  ...
  const geojson_endpoint = "/wp-json/ILEK-demo/geojson";
  const json_w_geocode = await get_geojson(geojson_endpoint);

  const info_json_endpoint = "/wp-json/ILEK-demo/infojson";
  ...
  function createListItem({
    ...
    let htmltext =
      ...
      '<img src="/wp-content/plugins/ILEK-demo/icons/' +
      category_shortname +
      '.svg"/>' +
      category_name +
      "</div>" +
  ...
  GetSearch.addEventListener("keyup", function () {
    ...
    jQuery.ajax({
      ...
      url: "wp-json/ILEK-demo/geojson?term=" + GetSearch.value,
      data: "",
      datatype: "html",
